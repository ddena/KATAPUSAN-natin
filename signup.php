...
$signup_error = false;
  ...
  if(isset($_POST['sub'])){
    ...
    $insertsql = "INSERT INTO tbl_user (full_name, username, email, password, role, otp, status) VALUES ('$SUfullname', '$SUusername', '$SUemail', '$SUpassword', '$role', '$SUotp', '$SUstatus')";

    if ($conn->query($insertsql) === TRUE) {
        $signup_success = true;

        $id = $conn->insert_id;
        $logssql = "INSERT INTO tbl_logs (user_id, action, datetime) VALUES ('$id', 'Signed Up', NOW())";
        $conn->query($logssql);
    }

    else if (!$conn->query($logssql)) {
        echo "Logs Insert Error: " . $conn->error;
      } else {
        $signup_error = true;
      }
